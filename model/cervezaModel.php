...
class cervezaModel
{
  ...
  function GetAllTable(){
    $sentencia = $this->db->prepare("SELECT c.id_cerveza, c.nombre,c.precio,d.id_creador,d.nombre as creador,d.localidad FROM cerveza c, distribuidor d WHERE c.id_creador = d.id_creador ORDER BY c.id_cerveza" );    
    $sentencia->execute();
    return $sentencia->fetchAll(PDO::FETCH_ASSOC);
  }
  ...
  function getDetalles($id_cerveza){
    $setencia = $this->db->prepare("SELECT c.nombre, c.precio, d.nombre as creador, d.localidad FROM cerveza c, distribuidor d WHERE c.id_cerveza = ?");
